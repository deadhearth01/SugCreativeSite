...
export default function SettingsPage() {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      else { const { error } = await res.json(); setToast({ message: error || 'Failed', type: 'error' }) }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <DashboardPanel title="Profile Settings">
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="email" value={profile.email || ''} disabled className="w-full border border-border rounded-lg px-4 py-2.5 text-sm bg-off-white text-foreground/50 cursor-not-allowed" />
            ...
            <div>
              ...
              <textarea value={profile.address || ''} onChange={e => setProfile(p => ({ ...p, address: e.target.value }))} className="w-full border border-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-[#35C8E0] resize-none" rows={2} placeholder="City, state or full address" />
            </div>
            <div>
        ...
        <DashboardPanel title="Preferences">
          <div className="space-y-4">
            ...
            <button className="bg-primary text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-primary/90 transition-colors">Save Preferences</button>
          </div>
        </DashboardPanel>
      </div>
