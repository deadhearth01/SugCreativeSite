  ...
  return (
    <div className="space-y-6">
      <div className="bg-[#1A9AB5] border border-border rounded-xl shadow-sm p-6">
        <div className="inline-flex items-center gap-2 bg-[#35C8E0] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 border border-white/20 rounded-lg shadow-sm mb-3">Client Portal</div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Projects', value: String(projectCount), icon: FolderKanban, color: 'bg-[#35C8E0]' },
          ...
          <div key={card.label} className="bg-white border border-border rounded-xl shadow-sm p-5">
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-9 h-9 ${card.color} border border-border rounded-xl flex items-center justify-center shadow-sm`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-border rounded-xl shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-[#1A9AB5]">
          ...
          {recentProjects.length === 0 ? <p className="text-sm text-foreground/50 py-8 text-center font-semibold">No projects yet.</p> : (
            <table className="w-full text-sm">
              <thead><tr className="bg-[#F4F6FA] border-b border-border">{['Project', 'Status', 'Deadline'].map(h => <th key={h} className="text-left py-3 px-4 text-[10px] font-black text-foreground/50 uppercase tracking-widest">{h}</th>)}</tr></thead>
              <tbody>{recentProjects.map((p, i) => (
                <tr key={i} className="border-b border-black/8 hover:bg-[#F4F6FA]">
                  <td className="py-3 px-4 font-bold text-[#1A9AB5] text-xs">{p.title}</td>
        ...
        <div className="bg-white border border-border rounded-xl shadow-sm">
          ...
          {recentPayments.length === 0 ? <p className="text-sm text-foreground/50 py-8 text-center font-semibold">No payments yet.</p> : (
            <table className="w-full text-sm">
              <thead><tr className="bg-[#F4F6FA] border-b border-border">{['Invoice', 'Amount', 'Status'].map(h => <th key={h} className="text-left py-3 px-4 text-[10px] font-black text-foreground/50 uppercase tracking-widest">{h}</th>)}</tr></thead>
              <tbody>{recentPayments.map((p, i) => (
                <tr key={i} className="border-b border-black/8 hover:bg-[#F4F6FA]">
                  <td className="py-3 px-4 font-mono text-xs text-foreground/50">{p.invoice_number || '—'}</td>
                  <td className="py-3 px-4 font-black text-[#1A9AB5] text-sm">₹{p.amount?.toLocaleString('en-IN') || 0}</td>
                  <td className="py-3 px-4"><StatusBadge status={p.status} /></td>
                </tr>
              ))}</tbody>
            </table>
